...
contract RandomnessReceiver {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) external {
        ...
        // assuming you want to use only the first numbers of the array 
        uint256 randomNumber = randomWords[0] % 6; //ensures number is between 0-5

        // Create a temporary array to hold the randomNumber
        uint256[] memory tempArray = new uint256[](1); // Create a new array with a single element
        tempArray[0] = randomNumber; // Set the first element to your randomNumber

        // Do your custom stuff here, for example:
        emit fulfilledRandomWords(requestId, tempArray);
    }
}